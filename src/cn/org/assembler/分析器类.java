    ...
    // 匹配立即数
      ...
      return Objects.equals(待匹配操作数类型, 目标操作数类型) 
          || (Objects.equals(待匹配操作数类型.寻址方式, 操作数元数据类.寻址方式_立即数)
              ...
              && ((操作数元数据类.类型16.equals(待匹配操作数类型.类型) && 操作数元数据类.类型16_32_可扩展到64.equals(目标操作数类型.类型))
                  // 0 匹配 Ib
                  || (操作数元数据类.类型8_有符号.equals(待匹配操作数类型.类型) && 操作数元数据类.类型8.equals(目标操作数类型.类型))));
    } else {
      // TODO: 待改进
      boolean 类型匹配 = Objects.equals(待匹配操作数类型.类型, 目标操作数类型.类型)
          || ((操作数元数据类.类型16.equals(待匹配操作数类型.类型) || 操作数元数据类.类型16_32_可扩展到64.equals(待匹配操作数类型.类型)) 
      ...
      boolean 寻址方式匹配 = Objects.equals(待匹配操作数类型.寻址方式, 目标操作数类型.寻址方式)
          || (待匹配操作数类型.寻址方式 != null 
              && 待匹配操作数类型.寻址方式.equals(操作数元数据类.寻址方式_寄存器) 
              && (目标操作数类型.寻址方式.equals(操作数元数据类.寻址方式_寄存器_ModRM)
                  || 目标操作数类型.寻址方式.equals(操作数元数据类.寻址方式_寄存器_ModRM_mod_通用寄存器)
                  || 目标操作数类型.寻址方式.equals(操作数元数据类.寻址方式_寄存器_ModRM_reg_通用寄存器)));
